   ...
   * With Google Sheet tracking for tutor visibility
   */

  // ===== GOOGLE SHEET TRACKING =====
  const PROGRESS_SHEET_URL = 'https://script.google.com/macros/s/AKfycbyIQm4AvRBNMX76vAMlObv4QOP7bZtdxxwe3k8BH1iFL87BTTGlRnZAUWgyXgrRthXX4w/exec';  // <-- Replace with your URL

  function getStudentEmail() {
      let email = localStorage.getItem('studentEmail');
      if (!email) {
          email = prompt('Enter your email to track progress:');
          if (email) localStorage.setItem('studentEmail', email);
      }
      return email;
  }

  function logToSheet(topic, activity, score) {
      ...
      fetch(PROGRESS_SHEET_URL, {
          method: 'POST',
          mode: 'no-cors',
          body: JSON.stringify({ email, topic, activity, score })
      });
  }
  // ===== END GOOGLE SHEET TRACKING =====
  ...
  const Progress = {
      ...
      completeActivity(activityId, score = null) {
          const data = this.getAll();
          const topicId = data.activities[activityId]?.topicId || 'Unknown';
          ...
          // Log to Google Sheet
          logToSheet(topicId, activityId, score);
